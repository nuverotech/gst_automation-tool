...
class SheetWriter:
    ...
    def _write_sheet_data(self, ws, df: pd.DataFrame, headers: List[str]):
        ...
        # 1. Define the 'unlocked' style
        unlocked = Protection(locked=False)
        
        # 2. Fill NaNs with empty strings. 
        # Writing 'None' or 'NaN' into a validated cell often breaks the dropdown.
        df = df.fillna("")
        ...
        for r_idx, row in df.iterrows():
            ...
            for c_idx, header in enumerate(headers, start=1):
                cell = ws.cell(row=excel_row, column=c_idx, value=row.get(header))

                # 3. CRITICAL: Explicitly unlock the data entry cells.
                # This allows Row 5+ to be editable while keeping Headers (1-4) protected.
    ...
    def build_and_write_all(self, df: pd.DataFrame) -> None:
        ...
        wb = load_workbook(self.output_path, keep_links=True)
